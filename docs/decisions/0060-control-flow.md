...
> Throwing exceptions can also be argued to be a `GOTO` statement, or some even like to ascribe it to a [`COMEFROM`](https://en.wikipedia.org/wiki/COMEFROM) statement for entertainment.
...
Every programming language and every runtime platform in commercial use today has its conventions, resources, communities, and ecosystems. These "conventions" are critical for "developer mobility", which is an (oft-underestimated) commercial reality in the commercial software world - often ignored by those who work outside that reality for their own convenience.
...
- Using result types for every layer in a large system is a useful pattern that must be enforced, but it must be used "all the way down" the call stack, to ensure correct usage and completeness.
...
At the moment in the .Net community, there is a resurgence of functional programming influence, and .Net is starting to support the use by the community of more and more of these concepts. Eventually, these concepts may permeate the entire community, and when that happens there will be much more tooling support and the conventions will change across the ecosystem. Being early to any new convention risks a reduction in the adoption of this codebase, for those unfamiliar with this convention.
