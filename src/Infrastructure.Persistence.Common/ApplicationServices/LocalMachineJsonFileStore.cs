...
{
    private const string PathSettingName = "Storage:LocalMachineJsonFileStore:RootPath";
        ...
        if (!Directory.Exists(path))
        {
            ...
            catch (Exception ex)
            {
                throw new InvalidOperationException(
                    Resources.LocalMachineJsonFileStore_VerifyRootPath_CreateFailed.Format(path), ex);
            }
        }
    }
...
internal static class LocalMachineFileStoreExtensions
{
    ...
    public static IReadOnlyDictionary<string, Optional<string>> ToFileProperties(this CommandEntity entity)
    {
        var containerProperties = entity.Properties
            .ToDictionary(pair => pair.Key, pair => pair.Value.ToFileProperty());
