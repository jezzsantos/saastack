...
public class PersonCredentialsApplicationSpec
{
    ...
    public async Task WhenAuthenticateAsyncWithCorrectPasswordAndMfa_ThenReturnsError()
    {
        ...
        result.Should().BeError(ErrorCode.ForbiddenAccess, Resources.PersonCredentialsApplication_MfaRequired,
            error => error.AdditionalCode == PersonCredentialsApplication.MfaRequiredCode
                     && error.AdditionalData!.Count == 1
                     && (string)error.AdditionalData[PersonCredentialsApplication.MfaTokenName] == "anmfatoken");
    ...
    [Fact]
    public async Task WhenGetPersonCredentialAsyncAndNotFound_ThenReturnsError()
    {
        _repository.Setup(s => s.FindCredentialByUserIdAsync(It.IsAny<Identifier>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Optional<PersonCredentialRoot>
                .None);

        var result = await _application.GetPersonCredentialAsync(_caller.Object, CancellationToken.None);

        result.Should().BeError(ErrorCode.EntityNotFound);
    }

    [Fact]
    public async Task WhenGetPersonCredentialAsync_ThenReturnsCredentials()
    {
        _caller.Setup(cc => cc.CallerId)
            .Returns("auserid");
        var credential = CreateVerifiedCredential();
