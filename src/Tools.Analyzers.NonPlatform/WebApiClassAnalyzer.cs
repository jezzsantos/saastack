...
///     SAS018: Error: There should not be an <see cref="AuthorizeAttribute" /> if the <see cref="RouteAttribute" />
///     declares <see cref="AccessType.Anonymous" /> access
///     SAS019: Warning: There should be a <see cref="AuthorizeAttribute" /> if the <see cref="RouteAttribute" /> declares
...
public class WebApiClassAnalyzer : DiagnosticAnalyzer
{
    ...
    internal static readonly DiagnosticDescriptor Sas018 = "SAS018".GetDescriptor(DiagnosticSeverity.Error,
        AnalyzerConstants.Categories.WebApi, nameof(Resources.SAS018Title), nameof(Resources.SAS018Description),
        nameof(Resources.SAS018MessageFormat));

    internal static readonly DiagnosticDescriptor Sas019 = "SAS019".GetDescriptor(DiagnosticSeverity.Warning,
        AnalyzerConstants.Categories.WebApi, nameof(Resources.SAS019Title), nameof(Resources.SAS019Description),
        nameof(Resources.SAS019MessageFormat));

    public override ImmutableArray<DiagnosticDescriptor> SupportedDiagnostics =>
        ImmutableArray.Create(Sas010, Sas011, Sas012, Sas013, Sas014, Sas015, Sas016, Sas017, Sas018, Sas019);
        ...
        AccessType access)
    {
        ...
        if (access == AccessType.Anonymous)
        {
            if (attribute is not null)
            {
                if (requestTypeSyntax.Type is IdentifierNameSyntax nameSyntax)
                {
                    context.ReportDiagnostic(Sas018, nameSyntax);
                }
        ...
        else
        {
            if (attribute is null)
            {
                if (requestTypeSyntax.Type is IdentifierNameSyntax nameSyntax)
                {
                    context.ReportDiagnostic(Sas019, nameSyntax);
                }
