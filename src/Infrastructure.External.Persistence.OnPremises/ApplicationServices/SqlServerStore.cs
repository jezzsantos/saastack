...
/// <summary>
///     Provides a combined store that persists all data to a remote SQL Server (hosted either locally).
/// </summary>
public sealed partial class SqlServerStore
{
    ...
    private readonly SqlServerStoreOptions.ConnectionOptions _connectionOptions;
    private readonly IRecorder _recorder;

    public static SqlServerStore Create(IRecorder recorder, SqlServerStoreOptions options)
    {
        return new SqlServerStore(recorder, options.Connection);
    }

    private SqlServerStore(IRecorder recorder, SqlServerStoreOptions.ConnectionOptions connectionOptions)
    {
        _recorder = recorder;
...
internal static class SqlServerStoreConversionExtensions
{
    ...
    {
        return dateTime >= SqlServerStore.MaximumAllowedSqlServerDate;
    }
    ...
    private static object ToTableEntityProperty(Optional<object> propertyValue, Optional<Type> targetPropertyType)
    {
        ...
        object ToDate(DateTime dateTime)
        {
            ...
            return dateTime.IsNotAllowedBySqlServerDate()
                ? SqlServerStore.MinimumAllowedSqlServerDate
    ...
    private static bool IsNotAllowedBySqlServerDate(this DateTime dateTime)
    {
        if (dateTime.HasValue())
        {
            return dateTime.ToUniversalTime() <= SqlServerStore.MinimumAllowedSqlServerDate;
        }
    ...
    {
        return dateTime <= SqlServerStore.MinimumAllowedSqlServerDate;
    }
