...
public class RabbitMqStoreOptions
{
    ...
    public static RabbitMqStoreOptions FromCredentials(IConfigurationSettings settings)
    {
        return new RabbitMqStoreOptions(
            settings.GetString(HostNameSettingName),
            settings.GetString(UserNameSettingName),
            settings.GetString(PasswordSettingName),
            settings.GetString(VirtualHostSettingName));
    }
