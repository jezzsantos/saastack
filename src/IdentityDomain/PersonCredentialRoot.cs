...
public sealed class PersonCredentialRoot : AggregateRootBase
{
    private const string CooldownPeriodInMinutesSettingName = "IdentityApi:PersonCredential:CooldownPeriodInMinutes";
    private const string MaxFailedLoginsSettingName = "IdentityApi:PersonCredential:MaxFailedLogins";
    private static readonly MfaOptions
        DefaultMfaOptions = MfaOptions.Create(false, true).Value; // EXTEND: Change default MFA options for all users
...
#pragma warning disable SAASDDD012
    public static Result<PersonCredentialRoot, Error> Create(IRecorder recorder, IIdentifierFactory idFactory,
#pragma warning restore SAASDDD012
    ...
    internal static Result<PersonCredentialRoot, Error> Create(IRecorder recorder, IIdentifierFactory idFactory,
        IConfigurationSettings settings, IEmailAddressService emailAddressService, ITokensService tokensService,
        IEncryptionService encryptionService, IPasswordHasherService passwordHasherService, IMfaService mfaService,
        Identifier userId, MfaOptions mfaOptions)
    {
        var root = new PersonCredentialRoot(recorder, idFactory, settings, emailAddressService, tokensService,
            encryptionService, passwordHasherService, mfaService);
        root.RaiseCreateEvent(IdentityDomain.Events.PersonCredentials.Created(root.Id, userId, mfaOptions));
        return root;
    }

    private PersonCredentialRoot(IRecorder recorder, IIdentifierFactory idFactory, IConfigurationSettings settings,
    ...
    {
        return (identifier, container, _) => new PersonCredentialRoot(container.GetRequiredService<IRecorder>(),
    ...
    public override Result<Error> EnsureInvariants()
    {
        ...
        {
            return Error.RuleViolation(Resources.PersonCredentialRoot_PasswordInitiatedWithoutRegistration);
        }
        ...
        Optional<EmailAddress> oobEmailAddress, Optional<EmailAddress> otpUsername, NotifyChallenged onChallenged)
    {
        ...
        {
            return Error.RoleViolation(Resources.PersonCredentialRoot_NotOwner);
        }
        ...
        if (!IsPasswordSet)
        {
            return Error.PreconditionViolation(Resources.PersonCredentialRoot_NoPassword);
        }

        if (!IsMfaEnabled)
        {
            return Error.PreconditionViolation(Resources.PersonCredentialRoot_MfaNotEnabled);
        }
        ...
        if (type is MfaAuthenticatorType.None or MfaAuthenticatorType.RecoveryCodes)
        {
            return Error.RuleViolation(Resources
                .PersonCredentialRoot_AssociateMfaAuthenticator_InvalidType);
        }
        ...
        if (!caller.IsAuthenticated)
        {
            if (MfaAuthenticators.HasAnyConfirmedPlusRecoveryCodes)
            {
                return Error.PreconditionViolation(Resources
                    .PersonCredentialRoot_AssociateMfaAuthenticator_MustChallenge);
            }
        }
        ...
        {
            return Error.PreconditionViolation(Resources
                .PersonCredentialRoot_AssociateMfaAuthenticator_AlreadyAssociated);
        }
        ...
        if (!authenticator.HasValue)
        {
            var added =
                RaiseChangeEvent(
                    IdentityDomain.Events.PersonCredentials.MfaAuthenticatorAdded(Id, UserId, type, true));
            if (added.IsFailure)
            {
                return added.Error;
            }
        ...
        {
            var recoveryAdded =
                RaiseChangeEvent(IdentityDomain.Events.PersonCredentials.MfaAuthenticatorAdded(Id, UserId,
    ...
    public Result<Error> ChangeMfaEnabled(Identifier modifierId, bool isEnabled)
    {
        ...
        return RaiseChangeEvent(
            IdentityDomain.Events.PersonCredentials.MfaOptionsChanged(Id, UserId, changed.Value));
    }
    ...
    public Result<Error> CompletePasswordReset(string token, string password)
    {
        ...
        if (password.IsInvalidParameter(pwd => _passwordHasherService.ValidatePassword(pwd, false),
                nameof(password), Resources.PersonCredentialRoot_InvalidPassword, out var error2))
        {
            return error2;
        }
        ...
        {
            return Error.PreconditionViolation(Resources.PersonCredentialRoot_PasswordResetTokenExpired);
        }

        var passwordHash = _passwordHasherService.HashPassword(password);
        var completed = RaiseChangeEvent(
            IdentityDomain.Events.PersonCredentials.PasswordResetCompleted(Id, token, passwordHash));
        ...
        {
            return RaiseChangeEvent(IdentityDomain.Events.PersonCredentials.AccountUnlocked(Id));
        }
        ...
        MfaAuthenticatorType type, string? oobCode, string confirmationCode)
    {
        ...
        if (!IsMfaEnabled)
        {
            return Error.RuleViolation(Resources.PersonCredentialRoot_MfaNotEnabled);
        }
        ...
        Identifier authenticatorId)
    {
        ...
        {
            return Error.RuleViolation(Resources
                .PersonCredentialRoot_DisassociateMfaAuthenticator_RecoveryCodesCannotBeDeleted);
        }

        var authenticatorDeleted = RaiseChangeEvent(
            IdentityDomain.Events.PersonCredentials.MfaAuthenticatorRemoved(Id, UserId, authenticator));
        ...
        if (MfaAuthenticators.HasOnlyRecoveryCodes)
        {
            var recoveryDeleted = RaiseChangeEvent(
                IdentityDomain.Events.PersonCredentials.MfaAuthenticatorRemoved(Id, UserId,
    ...
    public Result<string, Error> InitiateMfaAuthentication()
    {
        ...
        var raised =
            RaiseChangeEvent(
                IdentityDomain.Events.PersonCredentials.MfaAuthenticationInitiated(Id, UserId, initiated.Value));
        if (raised.IsFailure)
        {
            return raised.Error;
        }
    ...
    public Result<Error> ResetMfa(Roles resetterRoles)
    {
        ...
        {
            return Error.RoleViolation(Resources.PersonCredentialRoot_NotOperator);
        }
        ...
        return RaiseChangeEvent(IdentityDomain.Events.PersonCredentials.MfaStateReset(Id, UserId, DefaultMfaOptions));
    }

    public Result<Error> SetCredentials(string password)
    {
        if (password.IsInvalidParameter(pwd => _passwordHasherService.ValidatePassword(pwd, true),
                nameof(password), Resources.PersonCredentialRoot_InvalidPassword, out var error1))
        {
            return error1;
        }

        return RaiseChangeEvent(
            IdentityDomain.Events.PersonCredentials.CredentialsChanged(Id,
    ...
    public Result<bool, Error> VerifyPassword(string password, bool auditAttempt = true)
    {
        ...
        var raised =
            RaiseChangeEvent(
                IdentityDomain.Events.PersonCredentials.PasswordVerified(Id, isVerified, auditAttempt));
        if (raised.IsFailure)
        {
            return raised.Error;
        }
        ...
        {
            var locked = RaiseChangeEvent(IdentityDomain.Events.PersonCredentials.AccountLocked(Id));
            if (locked.IsFailure)
            {
                return locked.Error;
            }
        ...
        if (Login.HasJustUnlocked)
        {
            var unlocked = RaiseChangeEvent(IdentityDomain.Events.PersonCredentials.AccountUnlocked(Id));
    ...
    public Result<Error> VerifyRegistration()
    {
        ...
        return RaiseChangeEvent(IdentityDomain.Events.PersonCredentials.RegistrationVerificationVerified(Id));
    }
