...
#pragma warning restore SAASDDD012
    ...
    public async Task<Result<MfaAuthenticator, Error>> AssociateMfaAuthenticatorAsync(bool isAuthenticated,
        Identifier adderId, MfaAuthenticatorType type, Optional<PhoneNumber> oobPhoneNumber,
        Optional<EmailAddress> oobEmailAddress, Optional<EmailAddress> otpUsername, NotifyChallenged onChallenged)
    {
        ...
        if (authenticator is { HasValue: true, Value.HasBeenConfirmed: true })
        {
            return Error.PreconditionViolation(Resources
                .PasswordCredentialRoot_AssociateMfaAuthenticator_AlreadyExists);
        }

        if (!isAuthenticated
            && MfaAuthenticators.HasAnyConfirmedPlusRecoveryCodes)
        {
    ...
    public async Task<Result<MfaAuthenticator, Error>> ChallengeMfaAuthenticatorAsync(Identifier challengerId,
        Identifier authenticatorId, NotifyChallenged onChallenged)
    {
        if (!IsOwner(challengerId))
        {
            return Error.RoleViolation(Resources.PasswordCredentialRoot_NotOwner);
        }
    ...
    public Result<Error> ConfirmMfaAuthenticatorAssociation(Identifier completerId,
        MfaAuthenticatorType type, string? oobCode, string confirmationCode)
    {
        if (!IsOwner(completerId))
        {
            return Error.RoleViolation(Resources.PasswordCredentialRoot_NotOwner);
        }
    ...
    public Result<MfaAuthenticator, Error> DisassociateMfaAuthenticator(Identifier deleterId,
    ...
    public Result<Error> MfaAuthenticate(bool isAuthenticated, Optional<string> authenticationToken)
    {
        if (!IsRegistrationVerified)
        {
            return Error.PreconditionViolation(Resources.PasswordCredentialRoot_RegistrationUnverified);
        ...
        return MfaOptions.Authenticate(isAuthenticated, authenticationToken);
    }
    ...
    public Result<Error> VerifyMfaAuthenticator(bool isAuthenticated, Identifier verifierId,
        MfaAuthenticatorType type, string? oobCode, string confirmationCode)
    {
        if (isAuthenticated)
        {
    ...
    private Result<Error> DeleteAllMfaAuthenticators()
    {
        ...
        {
            var disassociated = DisassociateMfaAuthenticator(UserId, authenticator.Id);
