...
internal static class SqlServerQueryBuilderExtensions
{
        ...
        where TQueryableEntity : IQueryableEntity
    {
        ...
        {
            return $"{SqlServerStore.PrimaryTableAlias}.*";
        }
            ...
            case DateTime dateTime:
                if (dateTime.HasValue())
                {
                    return dateTime.IsMaximumAllowedSqlServerDate()
                        ? $"{columnName} {@operator} '{SqlServerStore.MaximumAllowedSqlServerDate:yyyy-MM-dd HH:mm:ss.fff}'"
                        : $"{columnName} {@operator} '{dateTime:yyyy-MM-dd HH:mm:ss.fff}'";
                }

                return
                    $"({columnName} {@operator} '{SqlServerStore.MinimumAllowedSqlServerDate:yyyy-MM-dd HH:mm:ss.fff}'"
    ...
    {
        return $"{SqlServerStore.JoinedEntityFieldAliasPrefix}{columnName}".ToColumnName();
    }
    ...
    {
        return $"[{tableName}] {SqlServerStore.PrimaryTableAlias}";
    }

    private static string ToAliasedColumnName(this string columnName)
    {
        return $"{SqlServerStore.PrimaryTableAlias}.{columnName.ToColumnName()}";
    }
