...
public sealed partial class SqlServerStore : IBlobStore
{
    ...
    public async Task<Result<Error>> DeleteAsync(string containerName, string blobName, CancellationToken cancellationToken)
    {
        ...
        return await ExecuteSqlDeleteCommandAsync(tableName, new KeyValuePair<string, object>("BlobName", blobName), cancellationToken);
    }
    ...
    public async Task<Result<Optional<Blob>, Error>> DownloadAsync(string containerName, string blobName, Stream stream, CancellationToken cancellationToken)
    {
        ...
        var tableName = await ConnectToBlobContainerAsync(containerName, cancellationToken);
        var queryResult = await ExecuteSqlSelectSingleCommandAsync(tableName, new KeyValuePair<string, object>("BlobName", blobName), cancellationToken);
    ...
    public async Task<Result<Error>> UploadAsync(string containerName, string blobName, string contentType, Stream stream, CancellationToken cancellationToken)
    {
        ...
        var deleteResult = await ExecuteSqlDeleteCommandAsync(tableName, new KeyValuePair<string, object>("BlobName", blobName), cancellationToken);
    ...
    {
        var sanitizedContainerName = containerName.SanitizeAndValidateStorageAccountResourceName();

        if (_cachedBlobContainerName != null &&
            string.Equals(_cachedBlobContainerName, sanitizedContainerName, StringComparison.InvariantCultureIgnoreCase))
        {
            ...
            if (existsObj == null)
            {
                var createSql = $@"
                        CREATE TABLE [dbo].[{sanitizedContainerName}] (
                            ...
                            CONSTRAINT [PK_{sanitizedContainerName}] PRIMARY KEY CLUSTERED ([BlobName] ASC)
                        )";
        ...
        {
            _recorder.TraceError(null, ex, "Failed to create or find table for container: {Container}", sanitizedContainerName);
            throw;
        }
