...
public class SqlServerStorageSpecSetup : StoreSpecSetupBase, IAsyncLifetime
{
    ...
    private async Task RebuildDatabaseSchema(string databaseName)
    {
        ...
        await _sqlServer.CopyAsync(Path.Combine(Environment.CurrentDirectory, "OnPremises", schemaScriptFileName),
            "/tmp/");
    ...
    public async Task InitializeAsync()
    {
        ...
        _store = SqlServerStore.Create(NoOpRecorder.Instance,
            SqlServerStoreOptions.CustomConnectionString(connectionString));
    }
