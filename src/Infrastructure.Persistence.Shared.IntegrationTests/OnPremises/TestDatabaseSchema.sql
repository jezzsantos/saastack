...
IF EXISTS(SELECT *
          FROM sys.objects
          WHERE object_id = OBJECT_ID(N'[dbo].[testblobstore]')
            AND type in (N'U'))
    DROP TABLE [dbo].[testblobentities]
GO
...
CREATE TABLE [dbo].[testblobstore] (
    [Id]          nvarchar(100) NOT NULL,
    [BlobName]    NVARCHAR(256) NOT NULL,
    [ContentType] NVARCHAR(256) NULL,
    [Data]        VARBINARY(MAX) NULL,
    CONSTRAINT [PK_testblobstore] PRIMARY KEY CLUSTERED ([Id] ASC)
);
GO