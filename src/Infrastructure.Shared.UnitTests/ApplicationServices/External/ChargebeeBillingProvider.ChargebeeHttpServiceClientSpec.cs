...
public class ChargebeeHttpServiceClientSpec
{
    ...
    public async Task WhenSubscribeAsyncAndImmediatelyWithDate_ThenReturnsError()
    {
        ...
        _serviceClient.Setup(x =>
                x.FindCustomerByIdAsync(It.IsAny<ICallerContext>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(Optional<Customer>.None);
        _serviceClient.Setup(x => x.CreateCustomerForBuyerAsync(It.IsAny<ICallerContext>(), It.IsAny<string>(),
        ...
        WhenSubscribeAsyncWithImmediatelyAndCustomerExists_ThenCreatesSubscriptionForCustomerAndUpdatesBillingDetails()
    {
        ...
        _serviceClient.Setup(x => x.UpdateCustomerForBuyerAsync(It.IsAny<ICallerContext>(), It.IsAny<string>(),
                It.IsAny<SubscriptionBuyer>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(customer);
        _serviceClient.Setup(x => x.UpdateCustomerForBuyerBillingAddressAsync(It.IsAny<ICallerContext>(),
                It.IsAny<string>(),
                It.IsAny<SubscriptionBuyer>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(customer);
        _serviceClient.Setup(x => x.CreateSubscriptionForCustomerAsync(It.IsAny<ICallerContext>(), It.IsAny<string>(),
        ...
        result.Value[Constants.MetadataProperties.CustomerId].Should().Be("acustomerid");
        result.Value[Constants.MetadataProperties.PaymentMethodStatus].Should()
            .Be(Customer.CustomerPaymentMethod.StatusEnum.Valid.ToString());
        result.Value[Constants.MetadataProperties.PaymentMethodType].Should()
            .Be(Customer.CustomerPaymentMethod.TypeEnum.Card.ToString());
        result.Value[Constants.MetadataProperties.SubscriptionId].Should().Be("asubscriptionid");
        result.Value[Constants.MetadataProperties.BillingPeriodValue].Should().Be("0");
        result.Value[Constants.MetadataProperties.BillingPeriodUnit].Should()
            .Be(Subscription.BillingPeriodUnitEnum.Month.ToString());
        result.Value[Constants.MetadataProperties.SubscriptionStatus].Should()
            .Be(Subscription.StatusEnum.Active.ToString());
        result.Value[Constants.MetadataProperties.SubscriptionDeleted].Should().Be(false.ToString());
    ...
    public async Task WhenChangeSubscriptionPlanAsyncAndActivated_ThenChangesPlan()
    {
        ...
        var datum = DateTime.UtcNow.ToNearestSecond();
        var customer = CreateCustomer("acustomerid");
    ...
    public async Task WhenChangeSubscriptionPlanAsyncAndUnsubscribed_ThenChangesPlan()
    {
        ...
        var datum = DateTime.UtcNow.ToNearestSecond();
        var customer = CreateCustomer("acustomerid");
        var subscription = CreateSubscription(customer, "asubscriptionid", status: Subscription.StatusEnum.UnKnown);
        var removedSubscription =
            CreateSubscription(customer, "asubscriptionid", status: Subscription.StatusEnum.Active);
        var changedSubscription = CreateSubscription(customer, "asubscriptionid", "aplanid2", datum, null, datum);
        _serviceClient.Setup(sc =>
    ...
    public async Task WhenCancelSubscriptionAsyncAndImmediate_ThenCancelsEndOfTerm()
    {
        ...
        var datum = DateTime.UtcNow.ToNearestSecond();
        var subscription = CreateSubscription(CreateCustomer("acustomerid"), "asubscriptionid", "aplanid", null, datum,
            datum);
        _serviceClient.Setup(sc =>
                sc.FindSubscriptionByIdAsync(It.IsAny<ICallerContext>(), It.IsAny<string>(),
                    It.IsAny<CancellationToken>()))
    ...
    private static Customer CreateCustomer(string customerId, bool hasPaymentMethod = false)
    {
        var customer = new
        {
            id = customerId,
