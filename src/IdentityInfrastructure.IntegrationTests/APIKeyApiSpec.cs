...
public class APIKeyApiSpec : WebApiSpec<Program>
{
    ...
    public async Task WhenCreateAPIKey_ThenCreatesNewKey()
    {
        ...
        var result = await Api.PostAsync(new CreateAPIKeyForCallerRequest(),
    ...
    {
        var login = await LoginUserAsync();
        var apiKey1 = (await Api.PostAsync(new CreateAPIKeyForCallerRequest(),
            req => req.SetJWTBearerToken(login.AccessToken))).Content.Value.ApiKey;
        var apiKey2 = (await Api.PostAsync(new CreateAPIKeyForCallerRequest(),
            req => req.SetJWTBearerToken(login.AccessToken))).Content.Value.ApiKey;
