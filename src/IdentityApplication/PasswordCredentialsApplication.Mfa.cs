...
partial class PasswordCredentialsApplication
{
    ...
    public async Task<Result<PasswordCredentialMfaAssociation, Error>> AssociateMfaAuthenticatorAsync(
    ...
    {
        var authenticated =
            await AuthenticateUserForMfaInternalAsync(caller, mfaToken, MfaPermittedAccessibility.Both,
                cancellationToken);
        ...
        var credential = authenticated.Value.Credential;
        var mfaCaller = authenticated.Value.Caller;
    ...
    public async Task<Result<PasswordCredentialMfaChallenge, Error>> ChallengeMfaAuthenticatorAsync(
        ICallerContext caller,
        string mfaToken, string authenticatorId, CancellationToken cancellationToken)
    {
        var authenticated = await AuthenticateUserForMfaInternalAsync(caller, mfaToken,
    ...
    public async Task<Result<PasswordCredentialMfaConfirmation, Error>> ConfirmMfaAuthenticatorAssociationAsync(
        ICallerContext caller,
        ...
        string confirmationCode, CancellationToken cancellationToken)
    {
        ...
        {
            return new PasswordCredentialMfaConfirmation
            {
                Tokens = null,
    ...
    {
        var authenticated = await AuthenticateUserForMfaInternalAsync(caller, null,
    ...
    private async Task<Result<(PasswordCredentialRoot Credential, MfaCaller Caller), Error>>
        AuthenticateUserForMfaInternalAsync(ICallerContext caller,
            string? mfaToken, MfaPermittedAccessibility accessibility, CancellationToken cancellationToken)
    {
...
{
    public static PasswordCredentialMfaAssociation ToAssociatedAuthenticator(
        ...
        IEncryptionService encryptionService)
    {
        var secret = authenticator.Type == MfaAuthenticatorType.TotpAuthenticator
            ...
            : null;
        return new PasswordCredentialMfaAssociation
        {
    ...
    public static PasswordCredentialMfaChallenge ToChallengedAuthenticator(this MfaAuthenticator authenticator)
    {
        return new PasswordCredentialMfaChallenge
        {
            OobCode = authenticator.OobCode,
            Type = authenticator.Type.ToEnum<MfaAuthenticatorType, PasswordCredentialMfaAuthenticatorType>()
        };
