...
partial class PersonCredentialsApplication
{
    ...
    public async Task<Result<PersonCredential, Error>> ChangeMfaAsync(ICallerContext caller,
        bool isEnabled, CancellationToken cancellationToken)
    {
        var retrievedCredential =
            await _repository.FindCredentialByUserIdAsync(caller.ToCallerId(), cancellationToken);
    ...
    public async Task<Result<List<CredentialMfaAuthenticator>, Error>> ListMfaAuthenticatorsAsync(
...
internal static class PersonCredentialMfaConversionExtensions
{
