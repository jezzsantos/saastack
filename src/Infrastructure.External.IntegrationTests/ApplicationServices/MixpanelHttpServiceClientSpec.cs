...
[Collection("EXTERNAL")]
public class MixpanelHttpServiceClientSpec : ExternalApiSpec
{
    private readonly MixpanelHttpServiceClient _serviceClient;

    public MixpanelHttpServiceClientSpec(ExternalApiSetup setup) : base(setup, OverrideDependencies)
    {
        var settings = setup.GetRequiredService<IConfigurationSettings>();
        _serviceClient = new MixpanelHttpServiceClient(NoOpRecorder.Instance, settings, new TestHttpClientFactory());
    }
