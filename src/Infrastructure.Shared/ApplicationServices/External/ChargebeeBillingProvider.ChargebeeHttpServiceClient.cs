...
public sealed partial class ChargebeeHttpServiceClient : IBillingGatewayService
{
    ...
    public ChargebeeHttpServiceClient(IRecorder recorder, IConfigurationSettings settings) : this(recorder,
        ...
        settings.Platform.GetString(StartingPlanIdSettingName), settings.Platform.GetString(ProductFamilyIdSettingName))
    {
    }
        ...
        CancelSubscriptionOptions options, BillingProvider provider, CancellationToken cancellationToken)
    {
        ...
        //TODO: make sure we dont lose any of the state when this call , for example any customer state
        ...
        ChangePlanOptions options, BillingProvider provider, CancellationToken cancellationToken)
    {
        ...
        var updatedState = retrievedSubscription.Value;
        ...
        var updatedStatus = retrievedStatus.Value.Status;
        ...
        //TODO: make sure we dont lose any of the state when this call returns , for example any customer state
        return updatedState;
    }
        ...
        TransferSubscriptionOptions options, BillingProvider provider, CancellationToken cancellationToken)
    {
        ...
        updatedState = updatedCustomer.Value;
        _recorder.TraceInformation(caller.ToCall(),
    ...
    {
        if (!state.TryGetValue(Constants.MetadataProperties.TrialEnd, out var trialEnd))
        {
    ...
    {
        if (state.TryGetValue(Constants.MetadataProperties.CustomerId, out var customerId))
        {
            return customerId;
        }
    ...
    {
        if (state.TryGetValue(Constants.MetadataProperties.SubscriptionId, out var subscriptionId))
        {
...
internal static class ChargebeeServiceClientConversionExtensions
{
    ...
    public static SubscriptionMetadata ToCustomerState(this Customer customer)
    {
        ...
        {
            { Constants.MetadataProperties.CustomerId, customer.Id }
        };
    ...
    {
        metadata[Constants.MetadataProperties.SubscriptionId] = subscription.Id;
        metadata.TryAdd(Constants.MetadataProperties.CustomerId, subscription.CustomerId);
        metadata.AppendPlanPeriod(subscription);
        metadata[Constants.MetadataProperties.SubscriptionStatus] = subscription.Status.ToString();
    ...
    {
        metadata.TryAddIfTrue(Constants.MetadataProperties.BillingPeriodValue, subscription.BillingPeriod,
            i => i.HasValue, i => i!.Value.ToString());
        metadata.TryAddIfTrue(Constants.MetadataProperties.BillingPeriodUnit, subscription.BillingPeriodUnit,
    ...
    private static void AppendPlan(this SubscriptionMetadata metadata, Subscription subscription)
    {
        if (subscription.SubscriptionItems.HasAny())
        {
            ...
            metadata[Constants.MetadataProperties.PlanId] = item.ItemPriceId;
        }
    ...
    private static void AppendInvoice(this SubscriptionMetadata metadata, Subscription subscription)
    {
        ...
        {
            var item = subscription.SubscriptionItems.First();
            metadata[Constants.MetadataProperties.BillingAmount] = item.Amount.GetValueOrDefault(0).ToString("G");
        }
    ...
    private static void AppendPaymentMethod(this SubscriptionMetadata metadata, Customer customer)
    {
        ...
        {
            metadata[Constants.MetadataProperties.PaymentMethodStatus] = customer.PaymentMethod.Status.ToString();
            metadata[Constants.MetadataProperties.PaymentMethodType] =
