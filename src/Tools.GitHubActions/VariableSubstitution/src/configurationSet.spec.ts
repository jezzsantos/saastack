import {ConfigurationSet, ConfigurationSetErrors} from "./configurationSet";
...
describe('verify', () => {
    ...
    it('should return true, when the set contains no required', async () => {
        ...
        expect(logger.info).toHaveBeenCalledWith(ConfigurationSetErrors.verificationSucceeded("apath"));
    });
    ...
    it('should return true, but warn, when the set defines required variable, but no variable exists to substitute', async () => {
        ...
        expect(result).toBe(true);
        expect(logger.warning).toHaveBeenCalledWith(ConfigurationSetErrors.redundantVariables(["arequired"]));
        expect(logger.info).toHaveBeenCalledWith(ConfigurationSetErrors.verificationSucceeded("apath"));
    });
    ...
    it('should return false, when the set defines required variable, but no variable/secret exists in GitHub', async () => {
        ...
        expect(logger.info).not.toHaveBeenCalledWith(ConfigurationSetErrors.verificationSucceeded("apath"));
        expect(logger.error).toHaveBeenCalledWith(ConfigurationSetErrors.missingRequiredVariables([{
            requiredVariable: "arequired-arequired:aname",
            gitHubVariableName: "AREQUIRED_AREQUIRED_ANAME"
        }]));
        expect(logger.error).toHaveBeenCalledWith(ConfigurationSetErrors.verificationFailed("apath"));
    });
    ...
    it('should return true, when the set defines required variable, and variable exists in GitHub', async () => {
        ...
        expect(result).toBe(true);
        expect(logger.info).toHaveBeenCalledWith(ConfigurationSetErrors.foundConfirmedVariables([{
            requiredVariable: "arequired-arequired:aname",
            gitHubVariableName: "AREQUIRED_AREQUIRED_ANAME"
        }]));
        expect(logger.info).toHaveBeenCalledWith(ConfigurationSetErrors.verificationSucceeded("apath"));
