...
namespace OnPremRelay.WorkerHost.Extensions;

public static class ServiceCollectionExtensions
{
    public static void AddMessageProcessing(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        services.AddSingleton<IServiceClientFactory>(c => new InterHostServiceClientFactory(
            c.GetRequiredService<IHttpClientFactory>(), c.GetRequiredService<JsonSerializerOptions>(),
        ...
        services.AddSingleton<QueueDelivery<AuditMessage>>(sp => new QueueDelivery<AuditMessage>(
            sp.GetRequiredService<IQueueMonitoringApiRelayWorker<AuditMessage>>(), WorkerConstants.Queues.Audits));

        services.AddSingleton<QueueDelivery<UsageMessage>>(sp => new QueueDelivery<UsageMessage>(
            sp.GetRequiredService<IQueueMonitoringApiRelayWorker<UsageMessage>>(), WorkerConstants.Queues.Usages));

        services.AddSingleton<QueueDelivery<ProvisioningMessage>>(sp => new QueueDelivery<ProvisioningMessage>(
            sp.GetRequiredService<IQueueMonitoringApiRelayWorker<ProvisioningMessage>>(),
            WorkerConstants.Queues.Provisionings));

        services.AddSingleton<QueueDelivery<EmailMessage>>(sp => new QueueDelivery<EmailMessage>(
            sp.GetRequiredService<IQueueMonitoringApiRelayWorker<EmailMessage>>(), WorkerConstants.Queues.Emails));

        services.AddSingleton<QueueDelivery<SmsMessage>>(sp => new QueueDelivery<SmsMessage>(
            sp.GetRequiredService<IQueueMonitoringApiRelayWorker<SmsMessage>>(), WorkerConstants.Queues.Smses));
