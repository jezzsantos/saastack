...
public class PersonCredentialProjection : IReadModelProjection
{
    private readonly IReadModelStore<PersonCredentialAuth> _credentials;
    private readonly IReadModelStore<MfaAuthenticator> _mfaAuthenticators;

    public PersonCredentialProjection(IRecorder recorder, IDomainFactory domainFactory, IDataStore store)
    {
        _credentials = new ReadModelStore<PersonCredentialAuth>(recorder, domainFactory, store);
    ...
    public Type RootAggregateType => typeof(PersonCredentialRoot);
}