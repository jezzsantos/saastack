...
public class MfaApi : IWebApiService
{
    ...
    public async
        Task<ApiPostResult<AssociatedPasswordCredentialMfaAuthenticator,
            ...
            AssociatePasswordMfaAuthenticatorForCallerRequest request, CancellationToken cancellationToken)
    {
        ...
        return () =>
            authenticator
                .HandleApplicationResult<AssociatedPasswordCredentialMfaAuthenticator,
    ...
    public async
        Task<ApiPutPatchResult<PasswordCredentialChallenge, ChallengePasswordMfaAuthenticatorForCallerResponse>>
            ...
            ChallengePasswordMfaAuthenticatorForCallerRequest request, CancellationToken cancellationToken)
    {
        var challenge =
            await _passwordCredentialsApplication.ChallengeMfaAuthenticatorAsync(_callerFactory.Create(),
                request.MfaToken, request.AuthenticatorId!,
                cancellationToken);
        return () =>
            challenge
                .HandleApplicationResult<PasswordCredentialChallenge,
    ...
    public async Task<ApiPutPatchResult<AuthenticateTokens, AuthenticateResponse>> ConfirmMfaAuthenticatorAssociation(
        ConfirmPasswordMfaAuthenticatorForCallerRequest request, CancellationToken cancellationToken)
    {
        var tokens = await _passwordCredentialsApplication.ConfirmMfaAuthenticatorAssociationAsync(
            _callerFactory.Create(), request.MfaToken,
